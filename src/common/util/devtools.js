...
function open(window=null, detached=false, dragable=false) {
  ...
  let devtools = new BrowserWindow({ 
    width : 0.4 * settings.display.width, // use 0.39 to not care about rounding issues
    height: window.getBounds().height 
  });
  
  let WindowAndDevToolFitScreen = () =>
      window.getBounds().width + devtools.getBounds().width >= settings.display.width;
  ...
  if ( WindowAndDevToolFitScreen() )
    devtools.setPosition(settings.display.width - devtools.getBounds().width - 1, window.getBounds().y)
  else {
    devtools.setPosition(window.getBounds().x + window.getBounds().width, window.getBounds().y)

    if ( dragable)
      window.on('move', () =>
        devtools && devtools.setPosition(window.getBounds().x + window.getBounds().width, window.getBounds().y) )
  }
